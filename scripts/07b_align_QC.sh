...
echo "samtools stats $file >$OUTDIR/${SAM}.dedup.stats"
done | \
parallel -j 15


# put the basic stats all in one file.---------------------------------------------------
FILES=($(find $OUTDIR -name "*.dedup.stats" | sort))

grep "^SN" ${FILES[0]} | cut -f 2 > $OUTDIR/SN.dedup.txt
for file in ${FILES[@]}
do paste $OUTDIR/SN.dedup.txt <(grep ^SN $file | cut -f 3) > $OUTDIR/SN2.dedup.txt && \
	mv $OUTDIR/SN2.dedup.txt $OUTDIR/SN.dedup.txt
	echo $file
done
